...
public class IndividualityConvert {
    ...
    public static String getdesc(String title,String level){
        String desc ="";
        if (descMap.get(title) == null){
            return desc;
        }
        switch (level){
            case "低分":
                return descMap.get(title).getLowDesc();
            case "中分":
                return descMap.get(title).getMiddleDesc();
            case "高分":
                return descMap.get(title).getHighDesc();
            case "是":
                return descMap.get(title).getLowDesc();
            case "否":
                return descMap.get(title).getHighDesc();
